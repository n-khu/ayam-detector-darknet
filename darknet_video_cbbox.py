...
class Ui_OutputDialog(QDialog):
    ...
    def cvDrawBoxes(self, detections, img):
        ...
        color_dict = {
            ...
            'ayam_makan' : [0, 123, 158], 'tempat_makan': [238, 123, 158], 'grup_ayam_makan' : [24, 245, 217], 'ayam_minum' : [224, 119, 227],
            'tempat_minum' : [154, 52, 104], 'grup_ayam_minum' : [179, 50, 247], 'ayam_diam' : [180, 164, 5], 'grup_ayam_diam' : [82, 42, 106]
        ...
        for label, confidence, bbox in detections:
            ...
            for name_key, color_val in color_dict.items():
                ...
                #     name_tag = name_tag +  " (" + str(round(bbox[0] * 100, 2)) + "%)"
                #     color = color_val 
                ...
                #                 (0,0,0), 2)
                    ...
                    if(self.makanCheckBox.isChecked()):
                        print("ayam makan are checked")
                            # name_tag = name_tag +  " (" + str(round(detection[1] * 100, 2)) + "%)"
                        if name_tag == 'ayam_makan':
                            name_tag = name_tag +  " (" + str(round(bbox[2], 2)) + "%)"
                            # color = color_val 
                            color = [0, 123, 158]
                            xmin, ymin, xmax, ymax = self.convertBack(
                            float(x), float(y), float(w), float(h))
                            ...
                            # Create bbox
                            cv2.rectangle(img, pt1, pt2, color, 1)
                            (test_width, text_height), baseline = cv2.getTextSize(name_tag, cv2.FONT_HERSHEY_SIMPLEX, 0.2, 1)
                            # Create filled bbox
                            cv2.rectangle(img,
                                pt1,(pt1[0] + test_width, pt1[1] - text_height - baseline),
                                color,
                            ...
                            cv2.putText(img,
                                        name_tag, 
                                        (pt1[0], pt1[1] - baseline), cv2.FONT_HERSHEY_SIMPLEX, 0.2,
                                        (0,0,0), 1)

                    if(self.minumCheckBox.isChecked()):
                        print("ayam minum are checked")
                            # name_tag = name_tag +  " (" + str(round(detection[1] * 100, 2)) + "%)"
                        if name_tag == 'ayam_minum':
                            name_tag = name_tag +  " (" + str(round(bbox[2], 2)) + "%)"
                            # color = color_val 
                            color = [0, 255, 0]
                            xmin, ymin, xmax, ymax = self.convertBack(
                    ...
                    if(self.diamCheckBox.isChecked()):
                        print("ayam diam are checked")
                        # name_tag = name_tag +  " (" + str(round(detection[1] * 100, 2)) + "%)"
                        if name_tag == 'ayam_diam':
                            name_tag = name_tag +  " (" + str(round(bbox[2], 2)) + "%)"
                            # color = color_val 
                            color = [123, 123, 158]
                            xmin, ymin, xmax, ymax = self.convertBack(
                        ...
                        # if(self.semuaCheckBox.isChecked()):
                        #     print("semua are checked")
                        #     # name_tag = name_tag +  " (" + str(round(detection[1] * 100, 2)) + "%)"
                        #     # if name_tag == 'ayam_diam':
                        #     name_tag = name_tag +  " (" + str(round(bbox[2], 2)) + "%)"
                        #     color = color_val 
    ...
    def YOLO(self, filename):
        ...
        configPath = "./yolo/yolov4-tiny-custom.cfg"
        weightPath = "./yolo/yolov4-tiny-custom_best.weights"
        metaPath = "./yolo/obj.data"
        ...
        if self.altNames is None:
            try:
                ...
                pass
        camAddress = 0 #for rtsp just put the links
        if filename == None :
            self.cap = cv2.VideoCapture(camAddress)
        else : 
        ...
        self.output_filename = "_output.avi"
        self.out = cv2.VideoWriter(
            self.output_filename, cv2.VideoWriter_fourcc(*"MJPG"), 1.0,
    ...
    def displayImage(self, frame_read, window=1):
        ...
        self.detections = darknet.detect_image(self.network, self.class_names, self.darknet_image, threshValue)
